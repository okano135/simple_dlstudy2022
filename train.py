...
Model_path = f'/raid/okano/model/txt_cls_t/{timestamp}'
...
for epoch in range(1, EPOCHS + 1):
    ...
    print('| end of epoch {:3d} | time: {:5.2f}s | '
          'valid accuracy {:8.3f} '.format(epoch, elapsed, accu_val))
    print('-' * 59)

    with open(Model_path+filename, mode='a') as f:
        f.write('-' * 59 + '\n')
        f.write('| end of epoch {:3d} | time: {:5.2f}s | '
                'valid accuracy {:8.3f} \n'.format(epoch, elapsed, accu_val))
        f.write('-' * 59 + '\n')

    Model_name = f'/cls_t_prms_{epoch}.pth'
    torch.save(model.state_dict(), Model_path+Model_name)

    # accu_test = evaluate(test_dataloader)
    # print('test accuracy {:8.3f}'.format(accu_test))
    # with open(Model_path+filename, mode='a') as f:
        # f.write('test accuracy {:8.3f}\n'.format(accu_test))
